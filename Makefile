...
$(PING_SUBNET_APP)_DEPS = $(PING_SUBNET_APP)/
$(PING_SUBNET_APP)_DEPS += $(patsubst %, $(PING_SUBNET_APP)/%.o, \
	main \
)

$(PING_SUBNET_GUI_APP)_DEPS = $(PING_SUBNET_GUI_APP)/
$(PING_SUBNET_GUI_APP)_DEPS += $(patsubst %, $(PING_SUBNET_GUI_APP)/%.o, \
	gui_app \
	gui_mainframe \
	output_stream_gui \
)
...
define BUILD_APP
CURRENT_TARGET = $(1).$(TARGET_EXT)
$(1): $$(CURRENT_TARGET)
$$(CURRENT_TARGET): $(BIN_DIR) $$(addprefix $(BUILD_DIR)/, $$($(1)_DEPS) $(COMMON_DEPS))
	$(CC) $(CFLAGS) $(CFLAGS_INCLUDE) $$(filter %.o, $$^) -o $(BIN_DIR)/$$@ $(OTHER_FLAGS)
endef
